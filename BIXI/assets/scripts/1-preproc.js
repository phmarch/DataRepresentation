...
function domainColor(color, data) {
	...
	var stations = data.map(function(d){return d.name});
...
function domainY(y, currentData) {
	...
	var min = 0;
	var max = d3.max(currentData.destinations, function(d){
		return d.count;
	});
	y.domain([min, max]);
 }
...
function getMatrix(data) {
	...
	return data.map(function(station){
		return station.destinations.map(function(dest) { 
			return dest.count;
		});
	});
}
...
function getTotal(data) {
	...
	return d3.sum(data, function(station){
		return d3.sum(station.destinations, function(dest) { 
			return dest.count;
		});
	});
}
